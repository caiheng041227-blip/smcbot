...
回测跨度 **1095 天(2023 → 2026 年中,约 3.5 年样本外)**,**已扣真实交易成本**
(往返手续费 + 滑点 —— 大多数散户回测悄悄省掉的那块):

| 净 @8bp | 净 @12bp | 净 @16bp |
|---:|---:|---:|
| +38.4R | **+33.1R** | +27.9R |

- 约 218 单,胜率约 34%,盈亏比约 2.3 —— edge 靠的是不对称,不是胜率。扣成本前是 +48.8R,
  差额就是手续费+滑点,用 `--cost-bp` 计入。
- **逐年(净 @12bp):** `2023 +7.6 · 2024 −6.3 · 2025 +7.4 · 2026 +24.3`。
  **依赖 regime** —— 2024 亏,2026 扛大头。照实放,不藏。
- **每单 Sharpe ≈ 0.12,p ≈ 0.07** —— *边缘显著*,未被确认。一个小幅正期望的研究系统,
  **不是印钞机**。
...
- **默认扣成本** —— 一个不算手续费和滑点的回测就是虚构。`--cost-bp` 按每单计入往返成本,
  所以头条数字是净值,不是毛值。
...
scripts/backtest.py           回测 harness(成本、回撤、盈利因子、逐年 OOS)
