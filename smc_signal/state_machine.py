...
class SignalState:
    ...
    # 2026-06-09:source_engine 标识信号来源 engine,用于:
    #   1. TG 推送时加 [SMC]/[ICT] 前缀
    #   2. tracker 决定用哪种 trail 模式(SMC=hybrid ATR / ICT=structural)
    #   3. 回测 / DB 分组统计
    # 默认 "smc"(SMC engine 出的信号);ICT engine 在创建时显式赋 "ict"
    source_engine: str = "smc"
    ...
    def _advance_on_15m(self, symbol: str, candle: Any) -> None:
        ...
        for sid, s in list(self.active_signals.items()):
            ...
            if s.step == SignalStep.STEP5_PASSED:
                # (2026-04-23 试过入场价距 POI 中线 1% 过滤,发现误伤赢家不救输家,已回滚。
                #  04-18/04-19 两条亏损 long 的 poi_distance_pct 实际分别是 0.31%/0.34%,
